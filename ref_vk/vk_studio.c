...
void R_StudioSetupBones( cl_entity_t *e )
{
	...
	if( pseqdesc->numblends > 1 )
	{
		...
		gEngine.R_StudioSlerpBones( m_pStudioHeader->numbones, q, pos, q2, pos2, s );
		...
		if( pseqdesc->numblends == 4 )
		{
			...
			s = (e->curstate.blending[0] * dadt + e->latched.prevblending[0] * (1.0f - dadt)) / 255.0f;
			gEngine.R_StudioSlerpBones( m_pStudioHeader->numbones, q3, pos3, q4, pos4, s );

			s = (e->curstate.blending[1] * dadt + e->latched.prevblending[1] * (1.0f - dadt)) / 255.0f;
			gEngine.R_StudioSlerpBones( m_pStudioHeader->numbones, q, pos, q3, pos3, s );
		}
	}
	...
	if( g_studio.interpolate && e->latched.sequencetime && ( e->latched.sequencetime + 0.2f > g_studio.time ) && ( e->latched.prevsequence < m_pStudioHeader->numseq ))
	{
		...
		if( pseqdesc->numblends > 1 )
		{
			...
			gEngine.R_StudioSlerpBones( m_pStudioHeader->numbones, q1b, pos1b, q2, pos2, s );
			...
			if( pseqdesc->numblends == 4 )
			{
				...
				gEngine.R_StudioSlerpBones( m_pStudioHeader->numbones, q1b, pos1b, q3, pos3, s );
			}
		}

		s = 1.0f - ( g_studio.time - e->latched.sequencetime ) / 0.2f;
		gEngine.R_StudioSlerpBones( m_pStudioHeader->numbones, q, pos, q1b, pos1b, s );
	}
	else
	{
